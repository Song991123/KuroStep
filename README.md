...
| 구분 | 링크 |
|---|---|
| Web UI | [GitHub Pages](https://song991123.github.io/KuroStep/) |
| Desktop Release | [GitHub Releases](https://github.com/Song991123/KuroStep/releases/tag/v0.1.0) |
...
| 영역 | 구현 내용 |
|---|---|
| Auth | 회원가입, 로그인, BCrypt 비밀번호 암호화, JWT 인증 |
| Task | 오늘 작업 카드 CRUD, `TODO` / `DOING` / `DONE` 상태 변경 |
| Playlist | YouTube 링크 기반 곡 등록, 플레이리스트 추가/삭제/순서 변경/셔플 |
| Work Context | 작업 카드와 플레이리스트 연결, 현재 작업 곡 지정 |
| Lyrics | LRCLIB 기반 가사 조회, 가사 라인 참조 생성, 로컬 캐시 우선 사용 |
| Translation Memo | 가사 라인별 번역 초안 및 사용자 메모 저장/삭제 |
| Desktop UI | Tauri shell, React 위젯, 작업 발자국 창, 가사 오버레이 창 |
| DevOps | GitHub Actions, Docker Compose, Terraform, AWS EC2 배포 구성 |
...
| 문서 | 내용 |
|---|---|
| [Project Overview](docs/project-overview.md) | 문제 정의, 사용자 흐름, MVP 범위 |
| [Features](docs/features.md) | 핵심 기능과 구현 상태 |
| [Architecture](docs/architecture.md) | 시스템 구조, 데이터 흐름, ERD |
| [Troubleshooting](docs/troubleshooting.md) | 개발 중 주요 문제와 해결 |
| [Release](docs/release.md) | 실행 방법, 배포, macOS Gatekeeper 안내 |
| [Final Report](docs/report/kurostep_v1_report.md) | 강사 제출용 최종 보고서 |
