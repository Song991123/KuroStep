...
- 플레이리스트 생성, 수정, 삭제
- 플레이리스트에 곡 추가 및 제거
- 플레이리스트 곡 순서 변경, 셔플, 현재 곡 제거 시 다음 곡 전환
- 작업 카드와 플레이리스트 연결
...
- GitHub Pages에 배포된 UI를 Tauri shell 안에서 로드하는 하이브리드 앱 구조
- 가사 로컬 캐시와 백그라운드 준비 전략
- Swagger/OpenAPI 문서화
- Docker 기반 실행 환경 구성
...
KuroStep은 가사 전문을 서버 DB에 계속 수집하는 구조를 피하고, 사용자의 클라이언트 로컬 캐시를 활용하는 방향으로 설계했습니다.

- 첫 재생 시: LRCLIB에서 가사를 조회하고 번역 초안을 준비합니다.
- 이후 재생 시: 로컬 캐시에 저장된 가사를 우선 사용해 바로 표시합니다.
- 재생 중: 다음 곡의 가사와 번역 초안을 백그라운드에서 미리 준비합니다.
- 서버 DB: 라인 메타데이터, 로컬 캐시 키, 사용자별 번역 메모를 저장합니다.

이를 통해 외부 API 호출로 인한 재생 지연을 줄이고, 서버가 가사 전문을 대량 보관하지 않도록 했습니다.
