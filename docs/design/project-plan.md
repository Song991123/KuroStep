...
- Swagger UI에서 전체 API 목록을 확인할 수 있게 한다.
- JWT 인증 API 테스트를 위해 Bearer Token 설정을 추가한다.
- 개발 중 요청/응답을 검증하는 문서로 활용한다.
...
- Docker Compose로 MySQL 컨테이너를 실행한다.
- Spring Boot 컨테이너와 MySQL 컨테이너를 연결한다.
- Swagger UI와 주요 API 동작을 확인한다.
...
- 서버 실행 로그를 확인한다.
- Swagger UI 접속을 확인한다.
- 회원가입, 로그인, 작업 생성 API를 테스트한다.
- Tauri 클라이언트에서 배포 서버 API를 호출해 확인한다.
...
| JWT 인증 후 사용자 정보가 null로 조회됨 | SecurityContext에 Authentication 저장 누락 |
| 다른 사용자의 작업을 수정할 수 있음 | 서비스 계층 권한 검증 누락 |
...
- SecurityConfig에서 인증 제외 경로와 인증 필요 경로를 명확히 분리한다.
- JWT 필터에서 토큰 검증 후 Authentication을 SecurityContext에 저장한다.
- 작업과 번역 메모 수정 시 로그인 사용자 id와 리소스 소유자 id를 비교한다.
- Service 계층에서 DTO 변환을 완료하거나 fetch join을 사용한다.
- Provider 인터페이스를 두어 LRCLIB 연동과 번역 API 연동을 분리한다.
- Docker Compose에서는 DB 호스트를 MySQL 서비스명으로 설정한다.
- Tauri 메인 위젯 안에 YouTube IFrame Player API 기반 플레이어를 렌더링하고, 재생/정지/스킵/진행바 이동 버튼이 플레이어를 제어하도록 구현한다.
...
- 따라서 로컬 번들 Tauri 앱에서는 YouTube 공식 iframe 재생이 제한될 수 있음을 기록하고, HTTPS로 배포된 프론트를 Tauri가 로드하는 구조를 대안으로 검토한다.
...
- 가사 위젯은 Tauri의 별도 `lyrics` 창을 두고, 메인 창에서 `set_lyrics_visible` 명령을 호출해 현재 가사 라인과 번역문을 `lyrics:update` 이벤트로 전달하는 방식으로 구현한다.
...
- Spring Security와 JWT 인증 흐름을 이해했다.
- JPA 연관관계를 사용해 작업, 곡, 가사 라인 참조, 번역 메모를 연결했다.
- 사용자별 데이터 권한 검증의 중요성을 확인했다.
- Provider 패턴으로 외부 API 연동을 분리하는 방법을 학습했다.
- Tauri 오버레이와 Spring Boot API를 연결하는 흐름을 경험했다.
- Docker와 EC2를 사용한 배포 흐름을 익혔다.
