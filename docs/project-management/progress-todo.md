...
GitHub Pages 배포와 백엔드 CI는 성공했고, Terraform으로 AWS EC2, 보안그룹, Elastic IP를 생성했다.
EC2에는 Docker Compose 기반으로 Spring Boot API와 MySQL 컨테이너를 실행했고, 외부에서 Swagger UI와 `/v3/api-docs` 접근을 확인했다.
...
EC2 배포 중 `t3.micro`에서 Spring Boot + MySQL 구동이 불안정해 `t3.small`로 조정했고, 배포 확인 후 비용 관리를 위해 인스턴스 종료 또는 `terraform destroy`가 필요하다.
...
EC2 Swagger /v3/api-docs HTTP 200
EC2 Swagger UI HTTP 200
EC2 Auth signup/login HTTP 200
```
...
| 문서/보고 | 최종 보고서 일부 현실화 | 최소 구현 완료 | `docs/report/final-report.md` | EC2 배포 확인 결과 반영 |
...
| 실행/배포 | EC2 운영 Docker Compose 구성 | 실제 검증 완료 | `KuroStep/Dockerfile.prod`, `KuroStep/docker-compose.prod.yml`, `.env.prod.example`, `application-prod.yaml` | 발표 후 비용 절감을 위해 서버 종료/삭제 |
| 실행/배포 | Terraform EC2 인프라 코드 | 실제 적용 완료 | `infra/main.tf`, `variables.tf`, `outputs.tf`, `user-data.sh`, `terraform.tfvars.example` | 배포 후 `terraform destroy` 절차 숙지 |
| 실행/배포 | Terraform 보안 파일 차단 | 실제 반영 완료 | `.gitignore` | `terraform.tfstate`, `*.tfvars`, `.env`, SSH key Git 추적 금지 유지 |
| 실행/배포 | EC2 SSH 키/로컬 tfvars 준비 | 로컬 준비 완료 / Git 제외 확인 | `~/.ssh/kurostep_ec2`, `infra/terraform.tfvars` | 민감정보 Git 추적 금지 유지 |
| 실행/배포 | GitHub Actions EC2 배포 workflow | 코드 준비 완료 | `.github/workflows/deploy-ec2.yml` | Secrets 등록 후 수동 배포 자동화로 전환 가능 |
...
| 실행/배포 | EC2 Swagger 외부 접속 | 실제 검증 완료 | `http://54.116.185.226:8080/swagger-ui/index.html`, `/v3/api-docs` | 발표 전 서버 유지 여부 확인 |
| 실행/배포 | EC2 Auth API 실요청 | 실제 검증 완료 | `/api/auth/signup`, `/api/auth/login` HTTP 200 | 토큰 값은 문서에 남기지 않음 |
| API 문서화 | Swagger 핵심 API 수동 확인 | 실제 검증 완료 | Swagger UI, `/v3/api-docs` | 발표 시 Swagger 화면 캡처 가능 |
| Auth/Security | 사용자별 권한 검증 보강 또는 문서화 | 필수 잔여 | Service 권한 검증 코드 | 남은 시간에 보강하거나 보고서에 한계 명시 |

## 현재 백엔드 완료 범위

2026-06-10 기준으로 아래 항목은 백엔드 보고 가능 범위에 포함한다.
...
| AWS/IaC | Terraform으로 EC2, 보안그룹, Elastic IP 생성 및 배포 확인 | 콘솔 수동 클릭이 아닌 코드 기반 인프라 관리 근거 |
| EC2 Deploy | Spring Boot + MySQL 컨테이너 실행, Swagger/Auth API 외부 확인 | 실제 배포 서버 동작 증거 확보 |

## 다음 작업 순서

| 우선순위 | 할 일 | 목표 | 최소 완료 기준 |
|---|---|---|---|
| 1 | 최종 보고서 최신화 | EC2 배포 성공 상태 반영 | Swagger/Auth API 200 확인 기록 |
| 2 | 발표 전 서버 상태 재확인 | 시연 실패 방지 | `docker compose ps`, Swagger 접속 확인 |
| 3 | HTTPS 후속 판단 | GitHub Pages와 API 연결 | 도메인/Nginx/Let's Encrypt 적용 여부 결정 |
| 4 | 비용 정리 | 불필요 과금 방지 | 발표 후 `terraform destroy` 또는 EC2 중지 |
...
- EC2 API는 `http://54.116.185.226:8080`에서 직접 확인했다.
- GitHub Pages와 연결하려면 HTTPS 설정이 필요하다.
- `t3.micro`에서는 Spring Boot 4 + MySQL 8 컨테이너 조합이 불안정해 SSH/API 응답이 끊기는 문제가 있었고, `t3.small`로 변경해 안정화했다.
- EC2 deploy workflow는 서버와 GitHub Secrets가 없을 때 push마다 실패하지 않도록 `workflow_dispatch` 수동 실행 전용으로 바꿨다.
- 현재 SSH 보안그룹은 배포 작업 편의를 위해 `0.0.0.0/0`으로 임시 개방되어 있으므로, 시연 후 개인 IP로 제한하거나 인스턴스를 제거해야 한다.
- `t3.small`은 비용이 발생하므로 발표/검증 이후 `terraform destroy`로 리소스를 정리한다.
- 이후 백엔드 구현 중 발생하는 Security, JWT, JPA, Provider, Docker 문제도 같은 방식으로 증상/원인/해결 과정을 누적 기록한다.

## 지금 당장 할 1개 작업

발표 전 `http://54.116.185.226:8080/swagger-ui/index.html` 접속과 `docker compose ps`를 한 번 더 확인한다.
